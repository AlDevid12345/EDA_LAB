...
x_tang = np.arange(0.25, 1.85, 0.02)
...
plt.plot(
    ...
    color = 'black',
    alpha=0.7,
    label = r'Tangent for threshold voltage at $Q_f$ = 0 $cm^{-2}$'
)
...
plt.plot(
    ...
    color = 'black',
    alpha=0.7,
    label = r'Tangent for threshold voltage at $Q_f$ =  $3.4 \times 10^{10}$ $cm^{-2}$'
)
...
plt.text(
    0,-0.3,
    f'Threshold Voltage ($V_{{TH}}$) for $Q_f$ = $0$ $cm^{{-2}}$: '
    f'{v_th[0]:1.3f} V\n'
    f'Threshold Voltage ($V_{{TH}}$) for $Q_f$ =  $3.4 \\times 10^{{10}}$ $cm^{{-2}}$: '
    f'{v_th[1]:1.3f} V ',
    ha='left',
    va='bottom',
    fontsize=9,
...
plt.tight_layout()
# plt.savefig(
#     'Q10.2_Id_Vgs_qf=0_&_3.4e10_vds=0.4_tox=5nm.png',
#     dpi = 300
# )
plt.show()
